			...
			$raw_message = [
				'l10n'	=> [
					...
					'wrapper'	=> [
						'function' 	=> 'sprintf',
						'args' 		=> [
							'#<a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_view_order_url($order_id) . '">' . $order->get_order_number() . '</a>', wc_get_order_status_name( 'completed' ), 
							$shop_name
						]
					]
				]
		...
		if( isset( $_POST['product_id'] ) && !empty( $_POST['product_id'] ) ) {
			...
			if( !isset( $_SESSION['wcfm_pages'] ) || !isset( $_SESSION['wcfm_pages']['products'] ) || ( isset( $_SESSION['wcfm_pages'] ) && isset( $_SESSION['wcfm_pages']['products'] ) && !in_array( $product_id, $_SESSION['wcfm_pages']['products'] ) ) ) {
				...
				$wcfm_detailed_analysis = $wpdb->prepare( "INSERT into {$wpdb->prefix}wcfm_detailed_analysis 
																	...
																	VALUES
																	(%d, %d, %d, %d, %d, %s, %s, %s, %s, %s)",
																	0, 0, 1, $product_id, $post_author, $HTTP_REFERER, $ip_address, $_SCOUNTRY, $_SSTATE, $_SCITY);
