						...
						foreach ($gd_duration_val_min as $key => $value) { 
							if( $gd_duration_type[$key] == 'days' ){ ?>
									...
									<tr class="<?php echo intval($k%2) ? 'eve' : 'odd'; $k++; ?>">
										...
										<td data-title="<?php echo sprintf( esc_attr__( '%1$s from %2$s - %3$s days', 'ova-brw' ), esc_attr( $price_text ), esc_attr( $gd_duration_val_min[$key] ), esc_attr( $gd_duration_val_max[$key] ) ); ?>">
